        ...
        public void ImportarArquivoLogix()
        {
            ...
            try
            {
                ...
                while ((linha = sr.ReadLine()) != null)
                {
                    ...
                    mrpLogix.QtdPedido = Decimal.Parse(dados[6].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.QtdSolicitada = Decimal.Parse(dados[7].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.LocalProd = dados[8].ToString(); 
                    mrpLogix.QtdCancelada  = Decimal.Parse(dados[9].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.Preco = Decimal.Parse(dados[10].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.QtdAtendida = Decimal.Parse(dados[11].ToString(), new CultureInfo("en-US")); 
                    ...
                    mrpLogix.NumSequencia  = Decimal.Parse(dados[16].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.IPI = Decimal.Parse(dados[17].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.CodUsuario	 = dados[18].ToString(); 
                    mrpLogix.Tipo = dados[19].ToString(); 
                    mrpLogix.SaldoTotal	= dados[20].ToString() == "" ? 0 : Decimal.Parse(dados[20].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.NumSequenciaPed = Decimal.Parse(dados[21].ToString(), new CultureInfo("en-US")); 
                    mrpLogix.Saldo = Decimal.Parse(dados[22].ToString(), new CultureInfo("en-US")); 
